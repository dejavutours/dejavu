...
class PaymentService {
  ...
  /**
   * Records a manual payment (e.g., by admin) and updates booking.
   * @param {string} bookingId - The ID of the booking.
   * @param {number} amount - The payment amount.
   * @param {string} paymentMethod - The payment method.
   * @param {string} referenceId - Reference ID for the payment.
   * @param {string} notes - Additional notes.
   * @returns {Promise<Object>} - The created payment log.
   */
  static async recordManualPayment(bookingId, amount, paymentMethod, referenceId, notes) {
    try {
      // Validate inputs
      if (!bookingId || !amount || amount <= 0 || !paymentMethod) {
        throw new Error('Invalid input: bookingId, a positive amount, and paymentMethod are required');
      }
      ...
      const paymentLog = new PaymentLog({
        ...
        paymentDate: new Date(),
        status: 'success',
        transactionDetails: { referenceId, notes },
      });
