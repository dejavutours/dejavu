...
exports.postAddTours = async (req, res, next) => {
  ...
  if (ifexist.length > 0) {
    ...
    const bannerimage = image.filename || null;
    const destinations = req.body.destinations || null;
    ...
    const placestovisit = req.body.placestovisit || null;
    const activities = req.body.activities || null;
    const itinerary = req.body.itinerary || null;
    const things_to_carry = req.body.things_to_carry || null;
    const includenexclude = req.body.includenexclude || null;
    const package_cost = req.body.package_cost || null;
    const infonfaq = req.body.infonfaq || null;
    const Bookncancel = req.body.Bookncancel || null;
    const guidelines = req.body.guidelines || null;
    const citydetails = req.body.citydetails.flat() || null;
    const region = req.body.region || null;
    const altitude = req.body.altitude || null;
    const difficulty = req.body.difficulty || null;
    const season = req.body.season || null;
    const tourcategory = req.body.tourcategory || null;
    const BestSeason = req.body.BestSeason || null;
...
exports.getTourDetails = async (req, res, next) => {
  try {
    ...
    const tests = await Tours.find().distinct('name');
    // const x = JSON.parse(JSON.stringify(tripdetails));
    // console.log(x);
    ...
    // console.log(tripdetails[0]);
    res.render('pages/TripDetails', {
      trips: tripdetails[0],
      test: tests,
      citydetailsfiltered: citydetailsfiltered,
    });
  } catch (err) {
    console.log(err);
  }
...
exports.metadata = async (req, res, next) => {
  ...
  // console.log(data);
  res.render('pages/metadata', {
    data: data,
  });
};

exports.updatetourcategory = async (req, res, next) => {
  console.log(req.body);
  MetaData.findById({ _id: req.body.id }).then((data) => {
    data.tourcategory = req.body.tourcategory;
    data.save().then((data) => {
      console.log(data);
    });
  });
