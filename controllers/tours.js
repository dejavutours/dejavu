...
exports.postBookTrip = (req, res, next) => {
  ...
  .then(result => {
    ...
    let regbody = " emailid: " + mailid + 
                  " username: " + username +
                  " usergender: " + usergender +
                  " userbirth: " + userbirth +
                  " usercontact: " + usercontact +
                  " userdestination: " + userdestination +
                  "co-travellers: " + notravellers +
...
exports.getUpcomingTrips = async(req, res, next) => {
  ...
  const nexttrips = await Tours.find({ upcomingtrip :  { $exists: true, $not: {$size: 0} } });
  const tests = await Tours.find().distinct('name');
...
exports.getAccomodationsDetails = async(req, res, next) => {
  ...
  try{
    ...
    const tests = await Tours.find().distinct('name');
    res.render("pages/AccomodationsDetails" , { accod: accoddetails[0], test: tests, });
  } 
...
exports.getDownloadPDF = (req, res, next) => {
  ...
  .then((data) => {
    ...
    pdfDoc.moveDown();
    pdfDoc.moveDown();
    pdfDoc.fontSize(25).font('Times-Roman').fillColor('black').text('Trekker Details', { align : 'center'});
    pdfDoc.fontSize(15).text('-------------------------------------------------------', { align : 'center'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('name :             ' + data.name, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('email :            ' + data.emailid, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('contact :          ' + data.contact, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('gender :           ' + data.gender, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('destination :      ' + data.destination, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('trip date :        ' + data.tripdate, { align : 'left'});
    pdfDoc.moveDown();
    pdfDoc.fontSize(12).fillColor('black').text('birth date :       ' + data.birthdate, { align : 'left'});
...
exports.postBookDate = async(req, res,next) => {
  ...
  const tripnames = req.body.tripname;
  const tripdates = req.body.tripdate;
  const tests = await Tours.find().distinct('name');
  res.render("pages/booktrip", {
    tripname : tripnames,
    tripdate : tripdates,
...
exports.postDeleteRegistration = (req, res, next) => {
  ...
  bookings.findByIdAndDelete(regid)
  .then(() => {
...
exports.getSearchBlog = async(req, res, next) => {
  ...
  const searchresult = await Blogs.find({ "title" : { "$regex": searchtext, "$options": "imxs" }});
  const distinctTags = await Blogs.find().distinct('tag');
  const tests = await Tours.find().distinct('name');
  res.render("pages/blog", {
    blogs: searchresult,
...
exports.maketripPdf = async(req, res, next) => {
  var obj;
...
exports.postgeneratePdf = async(req, res, next) => {
  ...
  if(req.body.inclusion != undefined && req.body.inclusion != ''){
    ...
    var inclusion_stack = [{text:'Inclusion:', style: 'header2',margin: [0, 10, 0, 10]},
                      {      ul: addinclusion       },];
  ...
  if(req.body.exclusion != undefined && req.body.exclusion != ''){
      ...
      var exclusion_stack = [{text:'Exclusion:', style: 'header2',margin: [0, 10, 0, 10]},
      {       ul: addexclusion     },];
  }
  ...
  if(req.body.add_trip_cost != undefined && req.body.add_trip_cost != ''){
      add_trip_cost =  req.body.add_trip_cost.split('#');
  ...
  if(req.body.act_attr != undefined && req.body.act_attr != ''){
      act_attr =  req.body.act_attr.split('#');
